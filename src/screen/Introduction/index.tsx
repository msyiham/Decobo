...
const Introduction = () => {
    ...
    useFocusEffect(
      useCallback(() => {
        ...
        const fetchData = async () => {
          try {
            const documentRef = doc(firestore, 'Introduction', 'Video'); // Specify the path separately
    ...
    useFocusEffect(
      useCallback(() => {
        ...
        const fetchData = async () => {
          try {
            const documentRef = doc(firestore, 'Introduction', 'description'); // Specify the path separately
            const documentSnapshot = await getDoc(documentRef);
            if (documentSnapshot.exists()) {
              const data = documentSnapshot.data(); // Retrieve the data from the document
              console.log(data);
  ...
  return (
    <ImageBackground source={require('../../asset/PAGE_BG/HOME.png')} resizeMode='cover' style={styles.container}>
      <ScrollView style={{padding:10}}>
        <Text style={styles.title}>Pengantar</Text>
        ...
        <View style={[styles.card,{borderRadius: 10, marginHorizontal: 5}]}>
          {description ? (
              <RenderHtml
...
const styles = StyleSheet.create({
    ...
    description:{
        fontFamily:theme.font2.regular, color: 'black', fontSize: 15, textAlign: 'justify',
    },
})